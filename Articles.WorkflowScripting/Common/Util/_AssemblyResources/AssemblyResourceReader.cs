...
namespace Common.Util
{
    public class AssemblyResourceReader
    {
        ...
        public AssemblyResourceReader(Type targetType)
        {
            ...
            m_targetAssembly = m_targetType.Assembly;

            m_resources = new List<AssemblyResourceInfo>();
            m_resourcesDict = new Dictionary<string, AssemblyResourceInfo>();
            foreach (AssemblyResourceFileAttribute actAttrib in m_targetType.GetCustomAttributes(s_attribType, true))
            {
                ...
                {
                    throw new ApplicationException("Resource " + actAttrib.ResourcePath + " not found!");
                }
            }
